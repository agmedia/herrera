...
class ControllerExtensionPaymentWSPay extends Controller {
  public function index() {
    ...
    $data['return_url'] = $this->url->link('extension/payment/wspay/callback');
            ...
            $data['IntCurrency'] = 'HRK';
            $IntAmount = $this->model_localisation_currency->getCurrencyByCode($data['IntCurrency']);
            $data['IntAmount'] = $IntAmount['value'] * $order_info['total'];
            $data['IntAmount'] = number_format($data['IntAmount'],2, ',', '');
