...
class ControllerExtensionMpImporterExporterImportProduct extends Controller {
	private $error = [];
	...
	public function getMenu() {
		...
		if ($this->user->hasPermission('access', 'extension/mpimporterexporter/import_product')) {
			$menu = array(
				'name'	   => $this->language->get('text_product_importer'),
				'href'     => $this->url->link('extension/mpimporterexporter/import_product', 'user_token=' . $this->session->data['user_token'], true),
				'children' => []
			);
		}
		return $menu;
	...
	public function index() {
		...
		$this->load->model('extension/mpimporterexporter/import_product');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/mpimporterexporter/import_product', '' . $url, true)
		);
		...
		$this->response->setOutput($this->load->view('extension/mpimporterexporter/import_product', $data));
	}
	...
	public function import() {
		...
		if (!$this->user->hasPermission('modify', 'extension/mpimporterexporter/import_product')) {
			...
			if (count($allDataInSheet) > 1) {
				foreach ($allDataInSheet as $default_value) {
							...
							foreach ($custom_fields_columnss as $columnkey => $custom_fields_column_field) {
								...
								if (isset($table_fieldnames[1])) {
									...
									if (count($table_fieldname) == 2) {
										$columns_info = $this->model_extension_mpimporterexporter_import_product->getColumns($table_fieldname[0], $table_fieldname[1]);
										if ($columns_info) {
					...
					if ($i != '0') {
						...
						if ($product_image) {
							if ((substr($product_image, 0, 7) == "http://" || substr($product_image, 0, 8) == "https://") && $this->model_extension_mpimporterexporter_import_product->file_contents_exist($product_image)) {
								  $imageString = file_get_contents($product_image);

								$dir_name = 'catalog/storeimages/';
								$folder_name = DIR_IMAGE. $dir_name;

								$this->model_extension_mpimporterexporter_import_product->makeDir($folder_name);
						...
						if ($find_importon == 'product_id') {
							$product_info = $this->model_extension_mpimporterexporter_import_product->getProductById($product_id);
						} else if ($find_importon == 'model_number') {
							$product_info = $this->model_extension_mpimporterexporter_import_product->getProductByModel($model_number);
						} else {
							$product_info = $this->model_extension_mpimporterexporter_import_product->getProductByName($product_name, $find_language_id);
						}

						if ($product_info) {
							// Update Exists Product
							if ($find_existsupdate) {
								$this->model_extension_mpimporterexporter_import_product->editProduct($product_info['product_id'], $insert_data, $find_data);
							...
							if ($find_importon == 'model_number' || $find_importon == 'product_name') {
								...
								$product_info_by_excelid = $this->model_extension_mpimporterexporter_import_product->getProductById($insert_data['product_id']);
							...
							$this->model_extension_mpimporterexporter_import_product->addProduct($insert_data, $find_data);
