...
class ControllerExtensionMpImporterExporterExportProduct extends Controller {
	private $error = [];
	private $oc_model_path = [];
	private $oc_controller_path = [];
	...
	public function __construct($registry) {
		...
		if (VERSION > '2.0.3.1') {
			...
			$this->oc_model_path['customer'] = [
				'path' => 'customer/customer',
				'obj' => 'model_customer_customer'
			];

			$this->oc_controller_path['customer_group'] = [
				'page' => 'customer/customer_group',
			];
			$this->oc_controller_path['customer'] = [
				'page' => 'customer/customer',
			];

			$this->oc_model_path['custom_field'] = [
	...
	public function getMenu() {
		...
		$menu = [];
		if ($this->user->hasPermission('access', 'extension/mpimporterexporter/export_product')) {
			$menu = [
				'name'	   => $this->language->get('text_product_exporter'),
				'href'     => $this->url->link('extension/mpimporterexporter/export_product', 'user_token=' . $this->session->data['user_token'], true),
				'children' => []
			];
		}
		return $menu;
	...
	public function index() {
		...
		$this->load->model('extension/mpimporterexporter/export_product');
		...
		$this->response->setOutput($this->load->view('extension/mpimporterexporter/export_product', $data));
	...
	protected function getFindFields() {
		...
		$extrafields = $this->model_extension_mpimporterexporter_export_product->getExtraFields();
	...
	public function export() {
		...
		// Fetch Products
		$results = $this->model_extension_mpimporterexporter_export_product->getProducts($filter_data);
		if (in_array($find_format, ['xls','xlsx','csv'])) {
			...
			if ($results) {
				...
				foreach ($results as $result) {
					...
					// Language
					$language_info = $this->model_extension_mpimporterexporter_export_product->getLanguage($result['language_id']);
					$result['language'] = ($language_info) ? $language_info['code']: '';

					// Store
					if (isset($find_store_id) && $find_store_id != '') {
						$stores = $this->model_extension_mpimporterexporter_export_product->getProductStores($result['product_id'], $find_store_id);
					} else {
						$stores = $this->model_extension_mpimporterexporter_export_product->getProductStores($result['product_id']);
					}
					...
					// Tax Class
					$tax_class_info = $this->model_extension_mpimporterexporter_export_product->getTaxClass($result['tax_class_id']);
					$result['tax_class'] = ($tax_class_info) ? $tax_class_info['title'] : '';

					// Stock Status
					$stock_status_info = $this->model_extension_mpimporterexporter_export_product->getStockStatus($result['stock_status_id'], $result['language_id']);
					$result['stock_status'] = ($stock_status_info) ? $stock_status_info['name'] : '';

					// Keyword
					$keyword_info = $this->model_extension_mpimporterexporter_export_product->getKeyword($result['product_id'], $result['language_id'], $find_store_id);
					$result['seo_url'] = ($keyword_info) ? $keyword_info['keyword'] : '';

					// Length Class
					$length_class_info = $this->model_extension_mpimporterexporter_export_product->getLengthClass($result['length_class_id'], $result['language_id']);
					$result['length_class'] = ($length_class_info) ? $length_class_info['title'] : '';

					// Weight Class
					$weight_class_info = $this->model_extension_mpimporterexporter_export_product->getWeightClass($result['weight_class_id'], $result['language_id']);

					$result['weight_class'] = ($weight_class_info) ? $weight_class_info['title'] : '';

					// Manufacturer
					$manufacturer_info = $this->model_extension_mpimporterexporter_export_product->getManufacturer($result['manufacturer_id'], $result['language_id'], $find_store_id);
					$result['manufacturer'] = ($manufacturer_info) ? $manufacturer_info['name'] : '';

					// Categories
					$categories_ids = $this->model_extension_mpimporterexporter_export_product->getProductCategories($result['product_id']);
					$result['categories_ids'] = ($categories_ids) ? implode(',', $categories_ids) : '';

					// Category Names
					$category_names = [];
					foreach ($categories_ids as $category_id) {
						$category_info = $this->model_extension_mpimporterexporter_export_product->getCategory($category_id, $result['language_id']);
						if ($category_info) {
					...
					// Filters
					$filter_ids = $this->model_extension_mpimporterexporter_export_product->getProductFilters($result['product_id']);
					$filter_names = [];
					foreach ($filter_ids as $filter_id) {
						$filter_info = $this->model_extension_mpimporterexporter_export_product->getFilter($filter_id, $result['language_id']);
						if ($filter_info) {
					...
					// Downloads
					$downloads = $this->model_extension_mpimporterexporter_export_product->getProductDownloads($result['product_id']);
					$download_names = [];
					foreach ($downloads as $download_id) {
						$download_info = $this->model_extension_mpimporterexporter_export_product->getDownload($download_id, $result['language_id']);
						if ($download_info) {
					...
					// Related Products
					$product_ids = $this->model_extension_mpimporterexporter_export_product->getProductRelated($result['product_id']);
					$result['related_products'] = ($product_ids) ? implode(',', $product_ids) : '';

					// Attribute
					$attributes = $this->model_extension_mpimporterexporter_export_product->getProductAttributes($result['product_id']);
					$attribute_names = [];
					foreach ($attributes as $attribute) {
						$attribute_info = $this->model_extension_mpimporterexporter_export_product->getAttribute($attribute['attribute_id'], $result['language_id']);
						if ($attribute_info) {
							$attribute_group_info = $this->model_extension_mpimporterexporter_export_product->getAttributeGroup($attribute_info['attribute_group_id'], $result['language_id']);
					...
					if ($find_image) {
						$images = $this->model_extension_mpimporterexporter_export_product->getProductImages($result['product_id']);
						$additional_images = [];
					...
					// Specials
					$specials = $this->model_extension_mpimporterexporter_export_product->getProductSpecials($result['product_id']);
					$specials_offers = [];
					...
					// Discount Offer
					$discounts = $this->model_extension_mpimporterexporter_export_product->getProductDiscounts($result['product_id']);
					$discounts_offers = [];
					...
					// Rewards
					$rewards = $this->model_extension_mpimporterexporter_export_product->getProductRewards($result['product_id']);
					$rewards_data = [];
					...
					// Options
					$options = $this->model_extension_mpimporterexporter_export_product->getProductOptions($result['product_id'], $result['language_id']);
					$options_data = [];
					...
					$result['reviews_data'] = '';
					if ($find_review) {
						$reviews = $this->model_extension_mpimporterexporter_export_product->getReviews($result['product_id'], $result['language_id']);
						$reviews_data = [];
					...
					$objPHPExcel->getActiveSheet()->setCellValue($char_value++ .$i, html_entity_decode($result_product_name, ENT_QUOTES, 'UTF-8'));
					...
					$objPHPExcel->getActiveSheet()->setCellValue($char_value++ .$i, html_entity_decode($result_description, ENT_QUOTES, 'UTF-8'));
					...
					$objPHPExcel->getActiveSheet()->setCellValue($char_value++ .$i, html_entity_decode($result_category_names, ENT_QUOTES, 'UTF-8'));
					...
					if (!empty($find_extrafields)) {
						foreach ($find_extrafields as $find_key_extrafield => $find_extrafield) {
							if (isset($find_fields[$find_key_extrafield]['code']) && isset($result[$find_extrafield['field']])) {
								$objPHPExcel->getActiveSheet()->setCellValue($char_value++ .$i, html_entity_decode($result[$find_extrafield['field']], ENT_QUOTES, 'UTF-8'));
							}
						}
					}
				}
				...
				foreach ($results as $result) {
					...
					if (isset($find_fields['description']['code'])) {
						$export_data[$i]['description'] = [
							...
							'value' => html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')
						];
					}
					...
					if (isset($find_fields['category_name']['code'])) {
						$export_data[$i]['category_names'] = [
							...
							'value' => html_entity_decode($result['category_names'], ENT_QUOTES, 'UTF-8')
						];
					}
						...
						foreach ($find_extrafields as $find_key_extrafield => $find_extrafield) {
							if (isset($find_fields[$find_key_extrafield]['code']) && isset($result[$find_extrafield['field']])) {
								$export_data[$i][$find_key_extrafield] = [
									...
									'value' => html_entity_decode($result[$find_extrafield['field']], ENT_QUOTES, 'UTF-8')
								];
							}
						}
					}
		...
		if ($results) {
			$json['href'] = str_replace('&amp;', '&', $this->url->link('extension/mpimporterexporter/export_product/fileDownload', 'user_token='. $this->session->data['user_token'] .'&file_name='. $file_name .'&find_format='. $find_format, true));
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/mpimporterexporter/export_product')) {
