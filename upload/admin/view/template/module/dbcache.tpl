      						...
      						<div class="tab-pane<?php echo ($tabkey == 0 ? ' active' : ''); ?>" id="tab-<?php echo str_replace('_', '-', $tab); ?>">
								...
								<?php foreach ($options as $field => $fieldtype) {
									...
									if (is_array($fieldtype)) {
        								...
        								<ul class="nav nav-tabs language-tabs multifield" id="<?php echo $extension_id; ?>-<?php echo $field_id; ?>-languages">
        									<?php foreach ($languages as $language) { ?>
        										<li><a href="#<?php echo $extension_id; ?>-<?php echo $field_id; ?>-<?php echo $language['language_id']; ?>" data-toggle="tab"><img src="<?php echo $language['flag']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
        									<?php } ?>
			      						</ul>
      									<div class="tab-content multifield">
											<?php foreach ($languages as $language) { ?>
												<div class="tab-pane" id="<?php echo $extension_id; ?>-<?php echo $field_id; ?>-<?php echo $language['language_id']; ?>">
			...
			<div class="col-sm-10">
				...
				<?php } elseif (($data['fieldtype'] == 'textarea') || ($data['fieldtype'] == 'multitextarea') || ($data['fieldtype'] == 'editor') || ($data['fieldtype'] == 'multieditor')) { ?>
					<?php if (($data['fieldtype'] == 'multitextarea') || ($data['fieldtype'] == 'multieditor')) { ?>
        				<ul class="nav nav-tabs language-tabs" id="<?php echo $extension_id; ?>-<?php echo $field_id; ?>-languages">
        					<?php foreach ($languages as $language) { ?>
        						<li><a href="#<?php echo $extension_id; ?>-<?php echo $field_id; ?>-<?php echo $language['language_id']; ?>" data-toggle="tab"><img src="<?php echo $language['flag']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
        					<?php } ?>
			      		</ul>
				...
				<?php } elseif (($data['fieldtype'] == 'autocomplete') || ($data['fieldtype'] == 'multiautocomplete')) { ?>
					<?php if ($data['fieldtype'] == 'multiautocomplete') { ?>
						...
						<div id="<?php echo $extension_id.'-'.$field_id; ?>" class="well well-sm autocomplete-results" style="height: 150px; overflow: auto;">
							<?php $items = (isset(${$data['fieldkey']}[$data['tab']]) ? ${$data['fieldkey']}[$data['tab']] : ${$data['fieldkey']}); ?>
							<?php foreach ($items as $item) { ?>
								<?php if (is_array($value) && in_array($item[0], $value)) { ?>
				...
				<?php } elseif ($data['fieldtype'] == 'image') { ?>						
   					<a href="" id="thumb-<?php echo $name; ?>" class="img-thumbnail" data-toggle="image"><img src="<?php echo ${$name.'_thumb'}; ?>" data-placeholder="<?php echo $placeholder; ?>" /></a>
   					<input type="hidden" name="<?php echo $name; ?>" value="<?php echo $value; ?>" id="input-<?php echo $name; ?>" />
				<?php } ?>
			</div>
		</div>
	<?php }
