  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form method="post" enctype="multipart/form-data" id="form-cigroupoption" class="form-horizontal">
...
$(document).delegate('#button-apply', 'click', function() {
  $.ajax({
    url: 'index.php?route=extension/cigroupprice_option/applyTemplate&<?php echo $module_token; ?>=<?php echo $ci_token; ?>',
    type: 'post',
    data: $("#form-cigroupoption").serialize(),
    dataType: 'json',
    beforeSend: function() {
    ...
    success: function(json) {
      ...
      if(json['error']) {
          ...
          if(json['error']['no_selected_manufacturer']) {
            ...
            if(json['error']['no_product_option']) {
              $('#tab-option').before('<div class="alert alert-danger">'+ json['error']['no_product_option'] +' <button type="button" class="close" data-dismiss="alert">&times;</button></div></div> ');
          }
      }

      if(json['success']) {
        $('#form-cigroupoption').before('<div class="alert alert-success"><i class="fa fa-exclamation-circle"></i> '+ json['success'] +' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
      }
    },
...
$('#input-template-id').on('change',function() {
  var template = $(this);
    ...
    $.ajax({
        url: 'index.php?route=extension/cigroupprice_option/getTemplate&<?php echo $module_token; ?>=<?php echo $ci_token; ?>&filter_id=' +  template.val(),
        dataType: 'html',
        beforeSend: function() {
...
//--></script>
<script type="text/javascript"><!--
$(document).delegate('.filter_option', 'click', function() {
...
$(document).delegate('#button-edit', 'click', function() {
  $.ajax({
    url: 'index.php?route=extension/cigroupprice_option/editTemplate&<?php echo $module_token; ?>=<?php echo $ci_token; ?>',
    type: 'post',
    data: $("#form-cigroupoption").serialize(),
    dataType: 'json',
    beforeSend: function() {
...
$(document).delegate('#button-add', 'click', function() {
  $.ajax({
    url: 'index.php?route=extension/cigroupprice_option/addTemplate&<?php echo $module_token; ?>=<?php echo $ci_token; ?>',
    type: 'post',
    data: $("#form-cigroupoption").serialize(),
    dataType: 'json',
    beforeSend: function() {
...
$(document).delegate('#button-delete', 'click', function() {
  ...
  $.ajax({
    url: 'index.php?route=extension/cigroupprice_option/deleteTemplate&<?php echo $module_token; ?>=<?php echo $ci_token; ?>',
    type: 'post',
    data: $("#form-cigroupoption").serialize(),
    dataType: 'json',
    beforeSend: function() {
