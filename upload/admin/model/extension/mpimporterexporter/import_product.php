<?php
use MpImporterExporter\Model;
class ModelExtensionMpImporterExporterImportProduct extends Model {
	...
	public function addProduct($data, $find_data) {
		...
		if(!empty($find_data['cell_operations']['manufacturer_id'])) {
			if(!empty($data['manufacturer_id'])) {
				$manufacturer_info = $this->db->query("SELECT manufacturer_id FROM " . DB_PREFIX . "manufacturer WHERE manufacturer_id = '" . (int)$data['manufacturer_id'] . "'")->row;
			} else{
			...
			if(!empty($manufacturer_info)) {
				...
				if($manufacturer_info) {
					...
					$manufacturer_id = $this->addManufacturer($manufacturerdata);
		...
		if (isset($find_data['store']) && !empty($find_data['cell_operations']['store'])) {
			foreach ($find_data['store'] as $store_id) {
				...
				if (!empty($data['seo_keyword']) && !empty($find_data['cell_operations']['seo_keyword'])) {
					...
					do {
						$exists = false;
						$url_alias_info = $this->getSeoUrlByKeyword($data['seo_keyword'], $find_data['language_id'] ,$store_id);
					...
					$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'product_id=" . (int)$product_id . "', language_id = '" . (int)$find_data['language_id'] . "', store_id = '" . (int)$store_id . "', keyword = '" . $this->db->escape(($data['seo_keyword'])) . "'");
				}
			}
		}		
		...
		if(!empty($find_data['cell_operations']['category_ids'])) {
			...
			if (!empty($data['category_ids'])) {
				foreach ($data['category_ids'] as $category_id) {
					$category_info = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$category_id . "'")->row;		
					if($category_info) {
				...
				foreach ($data['category_name'] as $category_name_value) {
					$category_name = explode('&gt;', $category_name_value);
					...
					if($category_info) {
						...
						$inserted_category_names = explode('&gt;', $category_name_value);
						$parent_id = 0;
						$number_row = 1;
						$allcat = count($inserted_category_names);
						foreach ($inserted_category_names as $inserted_category_name) {
							$inserted_category_name = trim($inserted_category_name);
							$category_info = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category_description WHERE name = '" . $this->db->escape($inserted_category_name) . "' AND language_id = '" . (int)$find_data['language_id'] . "'")->row;
							if($category_info) {
								$parent_id = $category_info['category_id'];
								$cat_id = $category_info['category_id'];
							} else{
								$category_description = array();
								$category_description[$find_data['language_id']] = array(
									'name'				=> $inserted_category_name,
									'description'		=> '',
									'meta_title'		=> $inserted_category_name,
									'meta_description'	=> '',
									'meta_keyword'		=> '',
								);
								...
								$cat_id = $this->addCategory($catdata);
								$parent_id = $cat_id;
							}

							if($number_row == $allcat && $cat_id) {
			...
			if (isset($find_data['store'])) {
				foreach ($find_data['store'] as $store_id) {
					...
					if(!empty($find_data['cell_operations']['seo_keyword'])) {
						$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'product_id=" . (int)$product_id . "' AND language_id = '" . (int)$find_data['language_id'] . "' AND store_id = '" . (int)$store_id . "'");
						...
						if (!empty($data['seo_keyword'])) {
							...
							$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET query = 'product_id=" . (int)$product_id . "', language_id = '" . (int)$find_data['language_id'] . "', keyword = '" . $this->db->escape(($data['seo_keyword'])) . "'");
						}
					}
				}
			}
	...
	public function getSeoUrl($seo_url_id, $language_id, $store_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE seo_url_id = '" . (int)$seo_url_id . "' AND language_id = '". (int)$language_id ."' AND store_id = '". (int)$store_id ."'");

		return $query->row;
	}

	public function getSeoUrlByKeyword($seo_keyword, $language_id, $store_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE keyword = '" . $this->db->escape($seo_keyword). "' AND language_id = '". (int)$language_id ."' AND store_id = '". (int)$store_id ."'");
	...
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "'");
		}
